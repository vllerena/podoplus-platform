...
// Lista de timezones válidas para IANA (subconjunto América Latina + UTC)
...
export class CreateBranchDto {
  @ApiProperty({ example: "SUC-001", description: "Código interno de la sede", required: false })
  @IsOptional()
  @IsString()
  @MaxLength(20, { message: "El código no puede superar 20 caracteres" })
  code?: string;

  @ApiProperty({ example: "Sede Central Lima", description: "Nombre de la sede" })
  @IsString()
  ...
  @ApiProperty({ example: 5, description: "Capacidad predeterminada de atención simultánea (1-50)" })
  ...
  @ApiProperty({ example: "America/Lima", description: "Zona horaria IANA de la sede" })
  @IsEnum(VALID_TIMEZONES, {
    message: `timezone debe ser una zona horaria válida. Opciones: ${VALID_TIMEZONES.join(", ")}`,
  })
  timezone: string;

  @ApiProperty({ description: "ID de la business unit a la que pertenece la sede", required: false })
  @IsOptional()
  @IsString()
